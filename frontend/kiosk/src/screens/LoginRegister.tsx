...
export function LoginRegister() {
  ...
  const [loginType, setLoginType] = useState<'aadhaar' | 'mobile' | null>(null);
  ...
  const loginFields = [
    { value: 'aadhaarNumber', label: t('aadhaarNumber') },
    { value: 'panNumber', label: t('panNumber') },
    { value: 'email', label: t('emailAddress') },
    { value: 'mobileNumber', label: t('mobileNumber') }
  ];
  ...
  const validateStep2 = () => {
    ...
    if (!formData.fatherName.trim()) newErrors.fatherName = t('fatherNameRequired');
    if (!formData.motherName.trim()) newErrors.motherName = t('motherNameRequired');
    if (!formData.aadhaarNumber.match(/^[0-9]{12}$/)) newErrors.aadhaarNumber = t('aadhaarInvalid');
    if (!formData.panNumber.match(/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/)) newErrors.panNumber = t('panInvalid');
  ...
  const validateStep3 = () => {
    ...
    if (!formData.pincode.match(/^[0-9]{6}$/)) newErrors.pincode = t('pincodeInvalid');
    if (!formData.occupation.trim()) newErrors.occupation = t('occupationRequired');
  ...
  const handleRegister = () => {
    // Automatically set login fields: Aadhaar Number and Mobile Number
    const registrationDataWithLogin = {
      ...formData,
      loginField1: 'aadhaarNumber',
      loginField2: 'mobileNumber'
    };
    
    setRegistrationData(registrationDataWithLogin);
    
    // Show success screen
    setShowSuccess(true);
    
    // Navigate after 2 seconds (SuccessScreen auto-completes)
    setTimeout(() => {
      setMode('login');
      setStep(1);
  ...
  const handleOtpVerify = () => {
    ...
    // Set user as authenticated in store
    const { setIsAuthenticated, setUser } = useStore.getState();
    setIsAuthenticated(true);
    setUser({
      name: 'User',
      aadhaarNumber: loginCredential.length === 12 ? loginCredential : '',
      consumerId: 'CONSUMER123',
      phoneNumber: loginCredential.length === 10 ? loginCredential : '',
      email: 'user@example.com'
    });
    // Verify OTP and navigate to service selection
                ...
                >
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-sm text-gray-600">{t('alreadyHaveAccount')}<br/>{t('loginToContinue')}</p>
                    </div>
                  </div>
                </button>
              ...
              {!loginType ? (
                <div className="space-y-4 mb-6">
                  ...
                  <button
                    ...
                    className="w-full bg-white border-2 border-gray-200 rounded-lg p-6 hover:border-[#F97316] hover:shadow-md transition-all group text-left"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 bg-[#F97316] bg-opacity-10 rounded-lg flex items-center justify-center group-hover:bg-opacity-20 transition-all">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 286 512.03" className="w-10 h-10">
                          <path fill="#43567C" fillRule="nonzero" d="M55.833 0h174.334C260.882 0 286 25.117 286 55.833v400.364c0 30.707-25.126 55.833-55.833 55.833H55.833C25.117 512.03 0 486.913 0 456.197V55.833C0 25.159 25.159 0 55.833 0z"/>
                          <path fill="#F97316" d="M276.681 70.044v371.209H9.318V70.044z"/>
                          <path fill="#FBBF24" fillRule="nonzero" d="M33.083 112.744c-5.543 5.542-13.972-2.887-8.43-8.429l18.661-18.661c5.543-5.543 13.972 2.887 8.43 8.429l-18.661 18.661zm4.432 34.078c-5.542 5.542-13.971-2.887-8.429-8.43l48.309-48.308c5.542-5.543 13.971 2.886 8.429 8.429l-48.309 48.309z"/>
                        </svg>
                      </div>
                      <div>
                        <p className="font-semibold text-[#212529] mb-1">{t('usingMobileNumber')}</p>
                        <p className="text-xs text-gray-600">{t('mobileNumberHelper')}</p>
                      </div>
                    </div>
                  </button>
                </div>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      {loginType === 'aadhaar' ? t('aadhaarNumberLabelRequired') : t('mobileNumberLabelRequired')}
                    </label>
                    <input
                      type="text"
                      value={loginCredential}
                      onChange={(e) => setLoginCredential(e.target.value)}
                      placeholder={loginType === 'aadhaar' ? t('aadhaarNumberHelper') : t('mobileNumberHelper')}
                      maxLength={loginType === 'aadhaar' ? 12 : 10}
              ...
              <div className="mb-6">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                  <p className="text-sm text-gray-700 text-center">
                    {t('otpSentTo')} <strong>{loginCredential}</strong>
                  </p>
                </div>
              ...
              <div key={index} className="flex items-center flex-1">
                <div className="flex flex-col items-center flex-1">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${
                    index + 1 < step ? 'bg-[#28A745] text-white' :
