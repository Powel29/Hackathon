...
export function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'complaints' | 'users' | 'kiosks'>('overview');
      ...
      <div className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex gap-1">
            {[
              { id: 'overview' as const, icon: <LayoutDashboard className="w-4 h-4" />, label: t('overview') },
              { id: 'complaints' as const, icon: <FileText className="w-4 h-4" />, label: t('complaintManagement') },
              { id: 'users' as const, icon: <Users className="w-4 h-4" />, label: t('userManagement') },
              { id: 'kiosks' as const, icon: <Monitor className="w-4 h-4" />, label: t('kioskStatus') }
            ].map((tab) => (
              <button
                key={tab.id}
                    ...
                    <tr key={complaint.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${
                          complaint.status === 'open' ? 'bg-orange-100 text-orange-700' :
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                        kiosk.status === 'online' ? 'bg-green-100' : 'bg-red-100'
                      }`}>
                        <Monitor className={`w-6 h-6 ${
                          kiosk.status === 'online' ? 'text-[#28A745]' : 'text-[#DC3545]'
                        }`} />
                      </div>
                      <div>
