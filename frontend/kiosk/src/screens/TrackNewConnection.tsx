...
import { ServiceType } from '../store/useStore';

interface NewConnectionApp {
  id: string;
...
export function TrackNewConnection() {
  ...
  const [selectedApp, setSelectedApp] = useState<NewConnectionApp | null>(null);
  const [error, setError] = useState('');

  const handleSearch = () => {
    const apps = JSON.parse(localStorage.getItem('newConnections') || '[]');
    const found = apps.find((app: NewConnectionApp) => app.applicationId === applicationId.toUpperCase());
  ...
  const getCategoryIcon = (serviceType: ServiceType) => {
    switch (serviceType) {
      case 'electricity':
  ...
  const getCategoryName = (serviceType: ServiceType) => {
    switch (serviceType) {
      case 'electricity':
