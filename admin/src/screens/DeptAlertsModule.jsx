                        ...
                        <div key={cat.key} className="card">
                            ...
                            <div className="card-body" style={{ padding: alerts.length === 0 ? 24 : 0 }}>
                                {alerts.length === 0 ? (
                                    ...
                                    alerts.map(a => (
                                        <div key={a.id} style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 14, opacity: deletingId === a.id ? 0.5 : 1 }}>
                                            <div style={{ flex: 1 }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                                                    <span style={{
                                                        padding: '2px 8px', borderRadius: 12, fontSize: 11, fontWeight: 700,
                                                        background: TYPE_STYLES[a.type]?.bg || '#f1f5f9',
                                                        color: TYPE_STYLES[a.type]?.color || '#475569',
                                                        border: `1px solid ${TYPE_STYLES[a.type]?.border || '#e2e8f0'}`
                                                    }}>{TYPE_STYLES[a.type]?.label || 'Info'}</span>
