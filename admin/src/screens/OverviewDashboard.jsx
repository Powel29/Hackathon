...
export function OverviewDashboard() {
    ...
    const activeCitizensToday = useMemo(() => {
        ...
        const ids = new Set();
        fComplaints.forEach(c => { if (new Date(c.createdAt).getTime() >= start)
            ids.add(c.citizenName); });
        fRequests.forEach(r => { if (new Date(r.createdAt).getTime() >= start)
            ids.add(r.citizenName); });
        return ids.size;
    ...
    const activityFeed = useMemo(() => {
        const complaintEvents = fComplaints.map(c => ({
            key: `c-${c.id}`,
            tone: '#DC3545',
            text: `${c.citizenName} registered ${c.complaintType} at ${c.location}`,
    ...
    return (<div className="space-y-4">
            ...
            <div className="grid-6 grid">
                {kpiCards.map(card => (<button key={card.key} title={card.tooltip} onMouseEnter={() => setHoveredKpi(card.key)} onMouseLeave={() => setHoveredKpi(null)} style={{
                textAlign: 'left',
            ...
            <div className="grid-3 grid">
                <div style={{ border: '1px solid #FECACA', background: '#FEF2F2', borderRadius: 12, padding: 12 }}>
                    ...
                    {criticalAlerts.length > 0 ? criticalAlerts.map(alert => (<div key={alert.id} style={{ marginBottom: 8, background: '#fff', borderRadius: 8, padding: 8, border: '1px solid #FECACA' }}>
                        ...
                        </div>)) : (<div style={{ fontSize: 12, color: '#7F1D1D' }}>No critical incidents right now.</div>)}
                </div>
                ...
                <div style={{ border: '1px solid #FED7AA', background: '#FFF7ED', borderRadius: 12, padding: 12 }}>
                    ...
                    {slaBreaches.length > 0 ? slaBreaches.map(alert => (<div key={alert.id} style={{ marginBottom: 8, background: '#fff', borderRadius: 8, padding: 8, border: '1px solid #FED7AA' }}>
                        ...
                        </div>)) : (<div style={{ fontSize: 12, color: '#9A3412' }}>No SLA breach right now.</div>)}
                </div>
                ...
                <div style={{ border: '1px solid #FDE68A', background: '#FEFCE8', borderRadius: 12, padding: 12 }}>
                    ...
                    {maintenanceWarnings.length > 0 ? maintenanceWarnings.map(kiosk => (<div key={kiosk.id} style={{ marginBottom: 8, background: '#fff', borderRadius: 8, padding: 8, border: '1px solid #FDE68A' }}>
                        ...
                        </div>)) : (<div style={{ fontSize: 12, color: '#854D0E' }}>No maintenance warnings right now.</div>)}
                </div>
            </div>
            ...
            <div className="grid-2 grid">
                <div className="card">
                    ...
                    <div className="card-body">
                        {chartsLoading ? (<ChartSkeleton />) : complaintVolumeTrend.some(d => d.complaints > 0) ? (<ResponsiveContainer width="100%" height={220}>
                            ...
                            </ResponsiveContainer>) : (<div style={{ height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: 12 }}>
                ...
                <div className="card">
                    ...
                    <div className="card-body">
                        {chartsLoading ? (<ChartSkeleton />) : resolutionStatusData.length > 0 ? (<ResponsiveContainer width="100%" height={220}>
                                <PieChart>
                ...
                <div className="card">
                    ...
                    <div className="card-body">
                        {chartsLoading ? (<ChartSkeleton />) : revenueVsTargetData.length > 0 ? (<ResponsiveContainer width="100%" height={220}>
                ...
                <div className="card">
                    ...
                    <div className="card-body">
                        {chartsLoading ? (<ChartSkeleton />) : departmentComparisonData.length > 0 ? (<ResponsiveContainer width="100%" height={220}>
            ...
            <div className="card">
                ...
                <div className="card-body" style={{ padding: 0 }}>
                    {activityFeed.length > 0 ? (<div style={{ maxHeight: 300, overflowY: 'auto' }}>
                            {activityFeed.map(item => (<div key={item.key} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '11px 16px', borderBottom: '1px solid #F1F5F9' }}>
                        ...
                        </div>) : (<div style={{ padding: 24, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>
                            No live activity available.
                        </div>)}
                </div>
            </div>

            {!hasAnyChartData && !chartsLoading && (<div className="card" style={{ padding: 16, textAlign: 'center', fontSize: 12, color: 'var(--text-muted)' }}>
