...
function BillAlertModal({ bill, onClose }) {
    const { sendBillAlert } = useAdminStore();
...
export function BillingModule() {
    ...
    const exportBillsCsv = () => {
        ...
        const headers = [
            'Bill Number', 'Department', 'Citizen', 'Consumer ID',
            'Billing Period', 'Amount', 'Due Date', 'Status',
            'Alert Sent', 'Alert Message', 'Alert Sent At',
        ];
        const rows = filtered.map(b => [
            b.billNumber, b.serviceType, b.citizenName, b.consumerId,
            b.billingPeriod, b.amount, new Date(b.dueDate).toISOString(), b.status,
            b.alertSent ? 'yes' : 'no', b.alertMessage, b.alertSentAt ? new Date(b.alertSentAt).toISOString() : '',
        ]);
        ...
        <div className="space-y-4">
            <div className="page-header">
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    ...
                    <button className="btn btn-outline btn-sm" onClick={exportBillsCsv} disabled={!canEditBilling}>
                            ...
                            <tr key={b.id} onClick={() => {
                                ...
                                <td onClick={e => e.stopPropagation()}>
                                    <div style={{ display: 'flex', gap: 6 }}>
                                        {b.status !== 'paid' && (
                                            <button className="btn btn-warning btn-sm" onClick={() => setAlertTarget(b)} title="Send payment alert" disabled={!canEditBilling}>
                                                📣 Alert
                                            </button>
                                        )}
                ...
                {sorted.length > 0 && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 12, borderTop: '1px solid var(--border)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--text-secondary)' }}>
                            ...
                            <span>{(currentPage - 1) * rowsPerPage + 1}-{Math.min(currentPage * rowsPerPage, sorted.length)} of {sorted.length}</span>
                        </div>
